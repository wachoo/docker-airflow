...
RUN  pip install -i http://mirrors.aliyun.com/pypi/simple  pytz pyOpenSSL \
    ndg-httpsclient \
    pyasn1 \
    apache-airflow[crypto,celery,postgres,hive,jdbc,mysql,ssh${AIRFLOW_DEPS:+,}${AIRFLOW_DEPS}]==${AIRFLOW_VERSION} \
   'redis==3.2' \
    ldap3
RUN if [ -n "${PYTHON_DEPS}" ]; then pip install -i http://mirrors.aliyun.com/pypi/simple  ${PYTHON_DEPS}; fi \
    && rm -rf \
        /tmp/* \
        /var/tmp/* 
